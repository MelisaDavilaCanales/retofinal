<template>
  <div class="p-4 bg-gray-100 mt-4 mb-8">
    <h2 class="text-lg font-mono font-semibold">Gender Stats</h2>
...
const { fetchGenderStatistics } = useStatistics();
...
const updateChartData = async () => {
  try {
    ...
    const dataJSON = await fetchGenderStatistics(`income=${incomeParam}`);

    const statistics = dataJSON.statistics.map((row: { option: string; percentage: string }): { option: string; percentage: number } => ({
      option: row.option,
